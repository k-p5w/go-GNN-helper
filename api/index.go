...
type ColorInfo struct {
	StrokeColor string // 事務所カラー（ふち用）
	TextColor   string // 文字色（基本白、太田プロのみ黒）
}
...
<svg width="%d" height="%d" viewBox="0 0 %d %d" xmlns="http://www.w3.org/2000/svg" style="background-color: transparent;">
    ...
    <g style="font-family:'Hiragino Kaku Gothic ProN','Meiryo',sans-serif; font-size:%dpx; font-weight:900; filter:url(#strongShadow);">
        ...
        <text x="45%%" y="50%%" text-anchor="middle" dominant-baseline="central" 
            stroke="%s" stroke-width="11" stroke-linejoin="round" paint-order="stroke" fill="%s">%s</text>
		...
		rippleRings.String(),
		currentFontSize, gi.Name,
		gi.ProductionColor.StrokeColor, gi.ProductionColor.StrokeColor, gi.Name,
		gi.ProductionColor.TextColor, gi.Name,
	)
...
func getProductionColor(name string) ColorInfo {
	...
	return ColorInfo{StrokeColor: itemColor, TextColor: textColor}
}
